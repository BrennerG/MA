...
class GATForMultipleChoice(torch.nn.Module):
    ...
    def __init__(self, params:{}):
        ...
        self.gatconv1 = GATConv(in_channels=self.embedding.dim, out_channels=params['gcn_hidden_dim'], heads=num_heads, dropout=params['dropout'])
        self.gatconv2 = GATConv(in_channels=params['gcn_hidden_dim']*num_heads, out_channels=1, concat=False, heads=1, dropout=params['dropout'])
    ...
    def forward(self, data):
        ...
        for i,answer in enumerate(data['answers']):
            ...
            x = F.relu(x)
            #x = F.dropout(x, training=self.training) # TODO is this needed? dropout already applied in gatconvs
